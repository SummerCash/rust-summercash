use super::super::client::CommunicationError;
...
impl<'a> Ctx<'a> {
    ...
    /// Puts the given response into the context buffer.
    pub fn put_response(&mut self, response: &'a [u8]) -> Result<(), CommunicationError> {
        *self.raw_response.lock()? = Some(response);
